...
static void relay_refresh_ui() {
  for (int i = 0; i < MAX_RELAYS; i++) {
    ...
    lv_label_set_text_fmt(relay_lbl[i], "%s\n%s", relay_cfg.names[i], on ? "ON" : "OFF");
  }
...
static void relay_rebuild_tab() {
  ...
  for (int i = 0; i < MAX_RELAYS; i++) relay_btn[i] = relay_lbl[i] = NULL;
  if (relay_cfg.addr) {
    for (int i = 0; i < relay_cfg.count; i++) {
      ...
      lv_obj_add_event_cb(b, relay_btn_cb, LV_EVENT_CLICKED, (void *)(intptr_t)i);
